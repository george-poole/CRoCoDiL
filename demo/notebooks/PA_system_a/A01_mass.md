...
With $\varphi=1$, $s_0 = s_r\text{H}(y-\zeta_0X) + \mathcal{N}_s$ and $c_0 = c_r\text{H}(y-\zeta_0X)+ \mathcal{N}_c$ on the domain $\Omega = [0, \mathcal{A}X] \times [0, X]$
...
With $\varphi=1$, $m_D^{\infty}=\text{vol}(\Omega)\langle c_\infty\rangle$ so by mass conservation on $\Omega = [0, \mathcal{A}X] \times [0, X]$ 
...
Substituting the total conserved mass and noting the physical constraint $m_C^{\infty}\geq0$ yields the same expression for the critical residual expression $s_r^*$.