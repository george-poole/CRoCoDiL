...
class FluxModelEquations:
    ...
    def ode_solve(
        ...
        lmbda,
        n,
        ics: tuple[float, float, float] | None,
        method: str,
    ) -> tuple:
        """
...
class FluxModel:
    ...
    lmbda: float
    n: float
    ics: InitVar[tuple[float, float, float] | None] = None
    method: InitVar[str] = 'RK45'

    def __post_init__(self, ics: tuple | None, method: str):
    ...
    def h(self) -> np.ndarray:
        """
        horizontally-averaged interface height `h(t)`
        """
